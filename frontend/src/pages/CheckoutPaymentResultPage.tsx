...
const content = {
  success: { title: "Pago recibido", text: "Estamos consultando el estado real al backend.", Icon: CircleCheck },
...
export default function CheckoutPaymentResultPage({ kind }: { kind: ResultKind }) {
  ...
  return <div className="section-shell"><div className="empty-state mx-auto max-w-2xl"><selected.Icon className="mx-auto mb-5 h-14 w-14 text-primary" /><h1 className="section-title">{selected.title}</h1><p className="mt-3 text-muted-foreground">{selected.text}</p>{error ? <p className="mt-5 text-destructive">{error}</p> : null}{order ? <div className="my-6 rounded-2xl bg-secondary p-5"><p className="font-bold">{order.orderNumber}</p><p className="mt-2">Estado del pago: <b>{order.paymentStatus}</b></p><p>Estado de la orden: <b>{order.status}</b></p><p className="mt-2 text-sm">Tracking: {order.trackingCode}</p></div> : <p className="my-6">Consultando estado real...</p>}<div className="flex flex-wrap justify-center gap-3">{order ? <Button asChild><Link to={`/pedido/${order.trackingCode}`}>Ver seguimiento</Link></Button> : null}{order && order.paymentStatus !== "APPROVED" ? <Button variant="outline" asChild><Link to={`/checkout/payment/${order.id}?${retryQuery}`}>Reintentar pago</Link></Button> : null}<Button variant="outline" asChild><Link to="/">Volver al inicio</Link></Button></div></div></div>;
}
