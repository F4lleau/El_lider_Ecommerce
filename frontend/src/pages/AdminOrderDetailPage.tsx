...
import { deliveryMethodLabel, orderStatusLabel, orderStatusOptionsForDelivery, paymentMethodLabel, paymentStatusLabel } from "@/utils/order-labels";

const transitionError = "No se puede cambiar el pedido a ese estado desde el estado actual.";

export default function AdminOrderDetailPage() {
  const id = Number(useParams().id);
  const { data: order, isLoading, error, reload } = useAdminData(() => adminOrdersApi.get(id));
  const [feedback, setFeedback] = useState("");
  const states = useMemo(() => order ? orderStatusOptionsForDelivery(order.deliveryMethod, order.status) : [], [order]);
  ...
  const change = async (status: OrderStatus) => {
    try {
      ...
      setFeedback(message.includes("No se puede cambiar") || message.includes("409") ? transitionError : message || "No se pudo actualizar");
    }
  };
  ...
  return <div>
    ...
    <div className="mt-6 grid gap-5 xl:grid-cols-[1fr_340px]">
      <section className="space-y-5">
        <div className="rounded-3xl border bg-card p-5"><h1 className="font-heading text-2xl font-bold">{order.orderNumber}</h1><p className="mt-1 text-sm text-muted-foreground">Número de seguimiento: {order.trackingCode}</p><div className="mt-5 grid gap-3 sm:grid-cols-3"><p><b>Cliente:</b><br />{name}<br />{email}</p><p><b>Estado:</b><br />{orderStatusLabel(order.status, order.deliveryMethod)}<br />Pago: {paymentMethodLabel(order.paymentMethod)}<br />{paymentStatusLabel(order.paymentStatus)}</p><p><b>Entrega:</b><br />{deliveryMethodLabel(order.deliveryMethod)}</p></div>{order.deliveryMethod === "SHIPPING" ? <p className="mt-5 rounded-xl bg-secondary p-4 text-sm">{order.shippingRecipient}, {order.shippingStreet} {order.shippingNumber}, {order.shippingCity}, {order.shippingProvince} ({order.shippingPostalCode})</p> : null}</div>
      ...
      <aside className="h-fit rounded-3xl border bg-card p-5"><h2 className="font-heading text-xl font-bold">Totales</h2><p className="mt-4 flex justify-between"><span>Subtotal</span><b>${Number(order.subtotal).toLocaleString("es-AR")}</b></p><p className="mt-2 flex justify-between"><span>Envío</span><b>${Number(order.shippingCost).toLocaleString("es-AR")}</b></p><p className="mt-4 flex justify-between text-xl"><span>Total</span><b>${Number(order.total).toLocaleString("es-AR")}</b></p><div className="mt-6"><label className="text-sm font-bold">Cambiar estado</label><select className="admin-select mt-2" value={order.status} onChange={(event) => void change(event.target.value as OrderStatus)}>{states.map((state) => <option key={state} value={state}>{orderStatusLabel(state, order.deliveryMethod)}</option>)}</select>{feedback ? <p className="mt-3 text-sm font-bold">{feedback}</p> : null}</div></aside>
    </div>
  </div>;
}
