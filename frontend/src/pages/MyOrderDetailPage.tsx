...
export default function MyOrderDetailPage() {
  ...
  return <div className="section-shell">{error ? <div className="empty-state text-destructive">{error}</div> : null}{order ? <><h1 className="section-title">{order.orderNumber}</h1><p className="mt-3">{order.status} · {order.paymentMethod} · {order.paymentStatus} · {order.deliveryMethod}</p><div className="mt-6 rounded-3xl border bg-card p-6">{order.items.map((item) => <p key={item.id}>{item.quantity} × {item.productName}{item.productSku ? ` (${item.productSku})` : ""} · ${Number(item.totalPrice).toLocaleString("es-AR")}</p>)}<p className="mt-5 font-heading text-xl font-bold">Total: ${Number(order.total).toLocaleString("es-AR")}</p></div></> : null}</div>;
}
