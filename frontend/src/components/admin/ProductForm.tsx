...
type TextFieldKey = "name" | "sku" | "slug" | "price" | "compareAtPrice" | "stock" | "image";
...
export function ProductForm({ product }: { product?: Product }) {
  ...
  useEffect(() => {
    void adminCategoriesApi.list().then(setCategories);
  }, []);
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!form.name || !form.categoryId || price <= 0 || Number(form.stock) < 0) {
      setError("Revisá nombre, categoría, precio y unidades disponibles.");
      return;
    }

    if (form.isOffer && (!compareAtPrice || compareAtPrice <= price)) {
      setError("En oferta, el precio anterior debe ser mayor.");
      return;
    }
  ...
  const text = (key: TextFieldKey, label: string, type = "text") => (
    <div>
      <Label>{label}</Label>
      <Input
        type={type}
        min={type === "number" ? 0 : undefined}
        step={key === "stock" ? 1 : type === "number" ? 0.01 : undefined}
        value={form[key]}
  ...
  return (
    <form onSubmit={submit} className="space-y-6">
      <div className="grid gap-4 rounded-3xl border bg-card p-5 sm:grid-cols-2">
        {text("name", "Nombre")}
        {text("sku", "SKU / Código interno")}
        {text("slug", "Slug")}
        {text("price", "Precio", "number")}
        {text("compareAtPrice", "Precio anterior", "number")}
        {text("stock", "Unidades disponibles", "number")}
        ...
        <div className="sm:col-span-2 flex flex-wrap gap-5">
          {(["isOffer", "isNew", "isFeatured", "isActive"] as const).map((key) => (
      ...
      <div className="flex gap-3">
        <Button disabled={loading}>{loading ? "Guardando..." : "Guardar producto"}</Button>
