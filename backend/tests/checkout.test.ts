...
const guestPayload = (deliveryMethod: DeliveryMethod) => ({ deliveryMethod, customer, items: [{ productId, quantity: 2 }], ...(deliveryMethod === DeliveryMethod.SHIPPING && { address }) });
...
before(async () => {
  ...
  const [product, inactive, noStock] = await Promise.all([
    prisma.product.create({ data: { name: `${marker}-product`, slug: `${marker}-product`, price: "125.50", stock: 20, categoryId: category.id } }),
...
describe("checkout invitado", () => {
  test("crea retiro con totales backend, snapshot y sin descontar stock", async () => {
    ...
    const order = result.body.data as { status: string; paymentStatus: string; shippingCost: string; subtotal: string; total: string; orderNumber: string; trackingCode: string; items: Array<{ productName: string; productSlug: string }> };
...
describe("checkout autenticado", () => {
  test("crea retiro asociado y vacia carrito", async () => {
    ...
    const result = await request("/api/checkout", userToken, { method: "POST", body: JSON.stringify({ deliveryMethod: DeliveryMethod.PICKUP, customer: { phone: "1112345678", name: "ignorado", email: "ignored@example.com" } }) });
    assert.equal(result.response.status, 201);
  ...
  test("crea envio asociado", async () => {
    await addAuthenticatedItem();
    const result = await request("/api/checkout", userToken, { method: "POST", body: JSON.stringify({ deliveryMethod: DeliveryMethod.SHIPPING, customer: { phone: "1112345678", name: "ignorado", email: "ignored@example.com" }, address }) });
    assert.equal(result.response.status, 201);
